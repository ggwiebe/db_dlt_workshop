...
-- MAGIC %md ### Context Raw - Access Stream  
-- MAGIC   
-- MAGIC **Common Storage Format:** CloudFiles, Kafka, (non-DLT) Delta tables, etc.  

-- COMMAND ----------

-- MAGIC %md ## 0. Data Profile  
-- MAGIC   
-- MAGIC ```
-- MAGIC CREATE TABLE ggw_retail_sandbox.customer_csv_table (
-- MAGIC        id INT,
...
-- MAGIC        channel INT,
-- MAGIC        active INT,
-- MAGIC        active_end_date TIMESTAMP,
-- MAGIC        update_dt TIMESTAMP,
-- MAGIC        update_user STRING)
-- MAGIC    USING CSV OPTIONS ( 'header' = 'true', 'inferSchema' = 'true', 'mode' = 'FAILFAST')
-- MAGIC LOCATION 'dbfs:/FileStore/tables/ggw_dlt_wshp/customer*.csv'
-- MAGIC ```
...
SELECT *,
  ...
  FROM cloud_files('/FileStore/tables/ggw_dlt_wshp/customer*.csv', 'csv', map('header', 'true', 'cloudFiles.inferColumnTypes', 'true') )
;

-- COMMAND ----------

-- MAGIC %md ### 1.b) Enriched Bronze  
-- MAGIC   
-- MAGIC Add row-level data lineage tracking from source file.
...
--     channel int,
--     active int,
--     active_end_date date   COMMENT 'Cast to Date from TS',
--     update_dt timestamp,
--     update_user string,
...
--           "CustomerMaster-Live" dlt_ingest_procedure,
...
-- MAGIC   
-- MAGIC Add a data mart based on an aggregate from silver.
